...
export default defineProject(({ mode }) => {
  ...
  // During build (especially on Cloudflare Pages), env vars might not be set.
  // Only throw in development mode. For production builds, use empty strings as defaults.
  // Note: VITE_* env vars are baked into the build, so set them in Cloudflare Pages build settings.
  ...
  publicEnvVars.forEach((key) => {
    if (!env[key]) {
      ...
      // For builds, use empty string (set actual values in Cloudflare Pages environment variables)
