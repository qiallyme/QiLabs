          ...
          {days.map((day, index) => {
            ...
            return (
              <Grid item xs={12 / 7} key={index}>
                <Box
                  onClick={() => {            const dayEvents = events.filter(event => isSameDay(parseISO(event.date), day));            setSelectedDate(day);            if (dayEvents.length === 0) {              console.log("Empty date clicked - ready for case creation:", format(day, "MMM d, yyyy"));              showNotification("Empty calendar date clicked! Case creation feature detected.", "info");            }          }}
                  sx={{
                    minHeight: 100,
                    p: 1,
                    border: 1,
