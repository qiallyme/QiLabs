...
auth_env = os.getenv('AUTHENTICATION_ENABLED')
auth_secret = os.getenv('AUTHENTICATION_SECRET_KEY')

if auth_env is not None:
    auth_enabled = auth_env.lower() in ('true', '1', 'yes')
    config['authentication']['enabled'] = auth_enabled
elif auth_secret:
    # Auto-enable auth if a secret key is provided (Production security)
    config['authentication']['enabled'] = True
    print("🔐 Authentication AUTO-ENABLED because AUTHENTICATION_SECRET_KEY is set")

auth_status = "ENABLED" if config.get('authentication', {}).get('enabled', False) else "DISABLED"
