...
// .wrangler/tmp/bundle-VDYZSL/strip-cf-connecting-ip-header.js
...
async function getCallerUser(env, accessToken) {
...
__name(getCallerUser, "getCallerUser");
function sbService(env) {
...
async function assertTenantAdmin(sb, tenantId, userId) {
...
__name(assertTenantAdmin, "assertTenantAdmin");
async function assertTenantMember(sb, tenantId, userId) {
  const { data, error } = await sb.rpc("is_tenant_member", { p_tenant: tenantId, p_user: userId });
  if (error)
    throw new Error(error.message);
  if (data !== true)
    throw new Error("Not authorized (member required).");
}
__name(assertTenantMember, "assertTenantMember");
async function ensureRole(sb, tenantId, name, rank = 50) {
  const { data, error } = await sb.from("roles").upsert({ tenant_id: tenantId, name, rank }, { onConflict: "tenant_id,name" }).select("id").single();
  if (error)
    throw new Error(error.message);
  return data.id;
}
__name(ensureRole, "ensureRole");
async function addMember(sb, tenantId, userId, displayName, status = "active") {
...
async function assignRole(sb, tenantId, userId, roleId) {
...
async function findRoleIdByName(sb, tenantId, name) {
  const { data, error } = await sb.from("roles").select("id").eq("tenant_id", tenantId).eq("name", name).single();
  if (error)
    throw new Error(`Role not found: ${name}`);
  return data.id;
}
__name(findRoleIdByName, "findRoleIdByName");
async function enableModules(sb, tenantId, moduleKeys, enabled) {
...
__name(enableModules, "enableModules");
async function setRoleAccess(sb, tenantId, moduleKey, roleId, access) {
...
__name(setRoleAccess, "setRoleAccess");
function generateICS(events) {
  let ics = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//QiChronicle//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH"
  ];
  for (const e of events) {
    ics.push("BEGIN:VEVENT");
    ics.push(`UID:${e.id}@qichronicle`);
    ics.push(`DTSTAMP:${(/* @__PURE__ */ new Date()).toISOString().replace(/[-:]/g, "").split(".")[0]}Z`);
    const start = new Date(e.start_at).toISOString().replace(/[-:]/g, "").split(".")[0] + "Z";
    ics.push(`DTSTART:${start}`);
    if (e.end_at) {
      const end = new Date(e.end_at).toISOString().replace(/[-:]/g, "").split(".")[0] + "Z";
      ics.push(`DTEND:${end}`);
    }
    ...
    if (e.kind) {
      ics.push(`CATEGORIES:${e.kind.toUpperCase()}`);
    }
    ics.push("END:VEVENT");
  }
...
var src_default = {
  async fetch(req, env) {
    ...
    try {
      ...
      const calendarMatch = path.match(/^\/calendar\/([0-9a-fA-F-]{36})\.ics$/);
      if (calendarMatch && req.method === "GET") {
        const secret = calendarMatch[1];
        const sb2 = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);
        const { data: events, error } = await sb2.schema("qichronicle").rpc("get_events_by_feed", { p_secret_key: secret });
        if (error || !events) {
        ...
        return new Response(icsBody, {
          headers: {
            ...
            "Content-Disposition": `attachment; filename="${secret}.ics"`
          }
        });
      }
      const token = getBearer(req);
      if (!token)
        return json({ error: "Missing Authorization: Bearer <access_token>" }, 401, cors);
      const caller = await getCallerUser(env, token);
      if (!caller?.id)
        return json({ error: "Invalid/expired token" }, 401, cors);
      ...
      if (path === "/api/bootstrap" && req.method === "POST") {
        ...
        const { data: t, error: e1 } = await sb.from("tenants").insert({ name, type, created_by: caller.id }).select("id").single();
        if (e1)
      ...
      if (membersMatch && req.method === "GET") {
        ...
        const { data: mem, error } = await sb.from("tenant_members").select("tenant_id,user_id,status,display_name,joined_at").eq("tenant_id", tenantId).order("joined_at", { ascending: true });
        if (error)
          throw new Error(error.message);
      ...
      if (inviteMatch && req.method === "POST") {
        ...
        if (!email)
          return json({ error: "email required" }, 400, cors);
        const redirectTo = env.SUPABASE_REDIRECT_TO || void 0;
        const { data: invited, error: invErr } = await sb.auth.admin.inviteUserByEmail(email, { redirectTo });
        if (invErr)
          throw new Error(invErr.message);
        const invitedUserId = invited.user?.id;
        if (!invitedUserId)
          throw new Error("Invite created but user id missing.");
      ...
      if (acceptMatch && req.method === "POST") {
        ...
        const { data: row, error } = await sb.from("tenant_members").select("status").eq("tenant_id", tenantId).eq("user_id", caller.id).maybeSingle();
        if (error)
          throw new Error(error.message);
        if (!row)
          throw new Error("No invite found for this tenant.");
        const body = await req.json();
        if (row.status === "invited") {
          const { error: upErr } = await sb.from("tenant_members").update({
            status: "active",
            display_name: body.display_name ?? null
          }).eq("tenant_id", tenantId).eq("user_id", caller.id);
          if (upErr)
      ...
      if (modListMatch && req.method === "GET") {
        ...
        const { data, error } = await sb.from("tenant_modules").select("module_key,is_enabled,settings,modules(name,description,route,icon,is_active)").eq("tenant_id", tenantId);
        if (error)
          throw new Error(error.message);
      ...
      if (modPatchMatch && req.method === "PATCH") {
        ...
        const body = await req.json();
        if (!body.module_key)
          throw new Error("module_key required");
        if (typeof body.is_enabled !== "boolean")
          throw new Error("is_enabled required");
      ...
      if (rolesListMatch && req.method === "GET") {
        ...
        await assertTenantAdmin(sb, tenantId, caller.id);
        const { data: roles, error: rErr } = await sb.from("roles").select("id,name,rank").eq("tenant_id", tenantId).order("rank", { ascending: true });
        if (rErr)
          throw new Error(rErr.message);
        const { data: access, error: aErr } = await sb.from("module_role_access").select("module_key,role_id,access").eq("tenant_id", tenantId);
        if (aErr)
      ...
      if (accessSetMatch && req.method === "POST") {
        ...
        const body = await req.json();
        if (!body.role_id)
          throw new Error("role_id required");
        if (!body.module_key)
          throw new Error("module_key required");
        if (!body.access)
...
// ../../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260303.0/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260303.0/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-VDYZSL/middleware-insertion-facade.js
...
// ../../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260303.0/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-VDYZSL/middleware-loader.entry.ts
